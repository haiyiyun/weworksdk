...
func (ww *weWork) GetUserBehaviorData(corpId string, filter GetUserBehaviorFilter) (resp GetUserBehaviorDataResponse) {
...
func (ww *weWork) GroupChatStatistic(corpId string, filter GroupChatStatisticFilter) (resp GroupChatStatisticResponse) {
	if ok := validate.Struct(filter); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
	}
...
func (ww *weWork) GroupChatStatisticGroupByDay(corpId string, filter GroupChatStatisticGroupByDayFilter) (resp GroupChatStatisticResponse) {
	if ok := validate.Struct(filter); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
	}
