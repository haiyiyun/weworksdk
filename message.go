...
func (ww *weWork) MessageSend(corpId string, msg interface{}) (resp MessageSendResponse) {
...
func (ww *weWork) MessageReCall(corpId string, msgId string) (resp internal.BizResponse) {
	h := H{"msgid": msgId}
	_, err := ww.getRequest(corpId).SetBody(h).SetResult(&resp).
