...
func (ww *weWork) TransferCustomer(corpId string, request TransferCustomerRequest) (resp TransferCustomerResponse) {
...
func (ww *weWork) TransferResult(corpId string, request TransferResultRequest) (resp TransferResultResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) GetUnassignedList(corpId string, request UnAssignedRequest) (resp UnAssignedResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) TransferCustomerResigned(corpId string, request TransferCustomerRequest) (resp TransferCustomerResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) TransferResultResigned(corpId string, request TransferResultRequest) (resp TransferResultResponse) {
	if ok := validate.Struct(request); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) TransferGroupChat(corpId string, request GroupChatTransferRequest) (resp GroupChatTransferResponse) {
	_, err := ww.getRequest(corpId).SetBody(request).SetResult(&resp).
