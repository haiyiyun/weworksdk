...
func (ww *weWork) GetJsApiTicket(corpId string) (resp TicketResponse) {
...
func (ww *weWork) GetJsApiAgentTicket(corpId string, agentId int) (resp TicketResponse) {
	var item *badger.Item
	var err error
	err = ww.cache.View(func(txn *badger.Txn) error {
...
func (ww *weWork) GetConfigSignature(corpId string, referer string) (resp JsTicketSignatureResponse) {
...
func (ww *weWork) GetAgentConfigSignature(corpId string, agentId int, referer string) (resp JsTicketSignatureResponse) {
	noncestr := randString(16)
	timestamp := time.Now().Unix()
	sl := []string{fmt.Sprintf("noncestr=%s", noncestr),
