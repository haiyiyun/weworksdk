...
func (ww *weWork) TagCreate(corpId string, tag Tag) (resp TagCreateResponse) {
...
func (ww *weWork) TagUpdate(corpId string, tag Tag) (resp internal.BizResponse) {
	if ok := validate.Struct(tag); ok != nil {
		resp.ErrCode = 500
		resp.ErrorMsg = ok.Error()
		return
...
func (ww *weWork) TagDelete(corpId string, id int) (resp internal.BizResponse) {
...
func (ww *weWork) TagList(corpId string) (resp TagListResponse) {
...
func (ww *weWork) TagUserList(corpId string, id int) (resp TagUserListResponse) {
	_, err := ww.getRequest(corpId).SetQueryParam("tagid", fmt.Sprintf("%v", id)).SetResult(&resp).
		Get("/cgi-bin/tag/get")
	if err != nil {
		resp.ErrCode = 500
...
func (ww *weWork) TagAddUsers(corpId string, tagId int, userIds []string, partyIds []int32) (resp TagAddOrDelUsersResponse) {
...
func (ww *weWork) TagDelUsers(corpId string, tagId int, userIds []string, partyIds []int32) (resp TagAddOrDelUsersResponse) {
	p := H{"tagid": tagId, "userlist": userIds, "partylist": partyIds}
	_, err := ww.getRequest(corpId).SetBody(p).SetResult(&resp).
