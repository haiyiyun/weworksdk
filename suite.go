...
func (ww *weWork) defaultAppSecretFunc(corpId string) (corpid string, secret string, customizedApp bool) {
...
// 默认从数据库获取应用的agentid
func (ww *weWork) defaultAgentIdFunc(corpId string) (appId int) {
	var authAgent models.Agent
	ww.engine.Model(models.Agent{}).
		Where(models.Agent{CorpId: corpId}).
		First(&authAgent)
...
func (ww *weWork) requestCorpToken(corpId string) (resp getCorpTokenResponse) {
	var err error
...
func (ww *weWork) SetAppSecretFunc(f func(corpId string) (corpid string, secret string, customizedApp bool)) {
	ww.getAppSecretFunc = f
}

func (ww *weWork) SetAgentIdFunc(f func(corpId string) (agentId int)) {
	ww.getAgentIdFunc = f
}

func (ww *weWork) getCorpToken(corpId string) (token string) {
	var err error
	var item *badger.Item
	err = ww.cache.View(func(txn *badger.Txn) error {
...
func (ww *weWork) GetUserInfo(corpId string, code string) (resp GetUserInfoResponse) {
...
func (ww *weWork) GetUserDetail(corpId string, userTicket string) (resp GetUserDetailResponse) {
...
// 　GET请求时data传入nil即可
// Deprecated:
func (ww *weWork) ExecuteCorpApi(corpId string, apiUrl string, query url.Values, data H) (body []byte, err error) {
